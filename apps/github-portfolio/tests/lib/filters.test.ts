...
import { applyTypeFilter, applyLanguageFilter, applySearchFilter, applySorting, filterRepositories } from "@/lib/filters";

const mockRepos = [
  { name: "repo-a", description: "A repo", language: "TypeScript", stargazers_count: 100, has_issues: true, has_wiki: true, open_issues_count: 5 },
  { name: "repo-b", description: null, language: "JavaScript", stargazers_count: 50, has_issues: false, has_wiki: false, open_issues_count: 0 },
  { name: "repo-c", description: "C repo", language: "TypeScript", stargazers_count: 200, has_issues: true, has_wiki: false, open_issues_count: 3 },
];
...
describe("filters", () => {
  ...
  describe("applyLanguageFilter", () => {
    ...
    it("returns all for unknown language", () => {
  ...
  describe("applySorting", () => {
    it("sorts by stars descending", () => {
      expect(applySorting(mockRepos as any, "stars_desc").map(r => r.stargazers_count)).toEqual([200, 100, 50]);
    });

    it("sorts by stars ascending", () => {
      expect(applySorting(mockRepos as any, "stars_asc").map(r => r.stargazers_count)).toEqual([50, 100, 200]);
    });

    it("sorts by name ascending", () => {
      expect(applySorting(mockRepos as any, "name_asc").map(r => r.name)).toEqual(["repo-a", "repo-b", "repo-c"]);
    });

    it("sorts by name descending", () => {
      expect(applySorting(mockRepos as any, "name_desc").map(r => r.name)).toEqual(["repo-c", "repo-b", "repo-a"]);
    });

    it("returns original for unknown sort", () => {
      expect(applySorting(mockRepos as any, "unknown" as any).map(r => r.name)).toEqual(["repo-a", "repo-b", "repo-c"]);
    });
  });
  ...
  describe("filterRepositories", () => {
    ...
    it("applies all filters combined", () => {
      const result = filterRepositories(mockRepos as any, "with_description", "TypeScript", "repo-a", "stars_desc");
